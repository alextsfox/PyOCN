...
ocn.fit_custom_cooling(lambda t: np.ones_like(t)*1e-7, pbar=True, n_iterations=16*16*100, max_iterations_per_loop=1)
...
plt.plot(ocn.history[:-1, 0], -np.diff(ocn.history[:, 1]))
